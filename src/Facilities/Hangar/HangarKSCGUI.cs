...
namespace KerbalKonstructs.UI2
{
    internal class HangarKSCGUI
    {
        ...
        internal static DialogGUIScrollList VaiantList
        {
            get
            {
                ...
                foreach (var staticInstance in StaticDatabase.allStaticInstances)
                {
                    ...
                    Modules.Hangar2 hangar = staticInstance.GetFacility(Modules.KKFacilityType.Hangar) as Modules.Hangar2;
                    foreach (Vessel storedVessel in hangar.storedVessels)
                    {
                        Vessel vessel = storedVessel;
                        list.Add(new DialogGUIHorizontalLayout(
                            new DialogGUILabel(staticInstance.groupCenterName ) ,
                            new DialogGUILabel(vessel.GetDisplayName()),
                            new DialogGUIButton("Launch", delegate { LaunchVessel(vessel); }, 140.0f, 23.0f, true))
                            );
                    }
                }
                list.Add(new DialogGUIFlexibleSpace());
        ...
        internal static void LaunchVessel(Vessel vessel)
        {
